...
# =============================================================================
CELLID = 8
STARTING_GRAY_LEVELS = [0.5]       # Starting point (uniform gray)

# Sample image configuration
USE_BIPARTITE_SAMPLE = True        # If True, use bipartite sample (two gray levels)
SAMPLE_GRAY_LEVEL = 0.5            # For uniform sample (when USE_BIPARTITE_SAMPLE=False)
SAMPLE_GRAY_LEFT = 0.25            # For bipartite sample: left half gray level
SAMPLE_GRAY_RIGHT = 0.75           # For bipartite sample: right half gray level

# Natural nudged mode configuration (takes precedence over bipartite/uniform)
USE_NATURAL_SMOOTHED = True        # If True, use natural image + nudged initial
NATURAL_IMG_IDX = None             # Index in remaining_idx (None = use start_img_idx)
NUDGE_AMOUNT = 0.1                 # Uniform nudge added to all pixels
...
print("-" * 40)
if USE_NATURAL_SMOOTHED:
    print(f"  Sample mode: NATURAL NUDGED (nudge={NUDGE_AMOUNT})")
    print(f"  Optimization iterations: {N_ITERATIONS}")
    print(f"  Learning rate: {LR}")
elif USE_BIPARTITE_SAMPLE:
    print(f"  Sample mode: BIPARTITE (left={SAMPLE_GRAY_LEFT*100:.0f}%, right={SAMPLE_GRAY_RIGHT*100:.0f}%)")
    print(f"  Starting gray levels: {[f'{g*100:.0f}%' for g in STARTING_GRAY_LEVELS]}")
    print(f"  Optimization iterations: {N_ITERATIONS}")
    print(f"  Learning rate: {LR}")
else:
    print(f"  Sample mode: UNIFORM ({SAMPLE_GRAY_LEVEL*100:.0f}%)")
    print(f"  Starting gray levels: {[f'{g*100:.0f}%' for g in STARTING_GRAY_LEVELS]}")
    print(f"  Optimization iterations: {N_ITERATIONS}")
    print(f"  Learning rate: {LR}")

# Pick a reference image index (used as the natural image in NATURAL_SMOOTHED mode)
if USE_NATURAL_SMOOTHED and NATURAL_IMG_IDX is not None:
    start_img_idx = remaining_idx[NATURAL_IMG_IDX].item()
else:
...
if USE_NATURAL_SMOOTHED:
    # Natural nudged mode: single optimization (no gray level loop)
    print(f"\n  --- Natural Nudged Mode (nudge={NUDGE_AMOUNT}) ---")

    debug_dict = {
        'DEBUG_SINGLE_IMAGE': True,
        'DEBUG_NATURAL_SMOOTHED': True,
        'nudge_amount': NUDGE_AMOUNT,
        'DEBUG_FIX_LAMBDA_i': True,  # Use posterior mean (no MC sampling)
        'verbose': True
    }
    ...
    results['natural_smoothed'] = result
    ...
    # Gray level modes: loop over starting gray levels
    for start_gray in STARTING_GRAY_LEVELS:
        print(f"\n  --- Starting gray: {start_gray*100:.0f}% ---")

        # Build DEBUG_dict based on sample mode
        if USE_BIPARTITE_SAMPLE:
            debug_dict = {
                'DEBUG_SINGLE_IMAGE': True,
                'DEBUG_FULL_FIELD_BIPARTITE': True,
                'start_gray': start_gray,
                'sample_gray_left': SAMPLE_GRAY_LEFT,
                'sample_gray_right': SAMPLE_GRAY_RIGHT,
                'verbose': True
            }
        else:
            debug_dict = {
...
for result_key, result in results.items():
    ...
    sample_img = result.get('sampled_img_pool')

    if USE_NATURAL_SMOOTHED:
        # Natural smoothed mode: sample is the original sharp image
        if sample_img is not None:
            sample_val = sample_img[0][mask].mean().item()
        else:
            sample_val = float('nan')
        start_pixel_val = initial_img[mask].mean().item() if initial_img is not None else float('nan')
    else:
        # Gray level modes
        start_gray = result_key
    ...
    if USE_NATURAL_SMOOTHED:
        ...
        print(f"\n  Starting {result_key*100:.0f}% gray (diagnostic start pixel val: {start_pixel_val:.4f}):")
...
n_rows = 1 if USE_NATURAL_SMOOTHED else len(STARTING_GRAY_LEVELS)
...
# Create sample image for visualization based on mode
mask_cpu = mask.cpu()

if USE_NATURAL_SMOOTHED:
    # Natural smoothed mode: sample is the original sharp natural image
    result = results['natural_smoothed']
    sample_img_pool = result.get('sampled_img_pool')
    if sample_img_pool is not None:
        sample_img_flat = sample_img_pool[0].cpu()
        sample_2d = sample_img_flat.reshape(n_px_side, n_px_side)
        sample_masked_mean = sample_img_flat[mask_cpu].mean().item()
    else:
        # Fallback (shouldn't happen)
        sample_img_flat = torch.zeros(n_px_side * n_px_side, device='cpu')
        sample_2d = sample_img_flat.reshape(n_px_side, n_px_side)
        sample_masked_mean = 0.0
    sample_title = f'Sample (original natural image)\nmask mean={sample_masked_mean:.3f}'
    sample_img_flat_cpu = sample_img_flat

elif USE_BIPARTITE_SAMPLE:
    # Recreate bipartite sample image for visualization
    col_indices = torch.arange(n_px_side, device='cpu').repeat(n_px_side, 1)
    col_indices_flat = col_indices.reshape(-1)
    masked_cols = col_indices_flat[mask_cpu]
    col_center = (masked_cols.min() + masked_cols.max()) / 2.0

    gray_left_value = (pixel_min + pixel_range * SAMPLE_GRAY_LEFT).cpu().item()
    gray_right_value = (pixel_min + pixel_range * SAMPLE_GRAY_RIGHT).cpu().item()

    is_left = (col_indices_flat < col_center) & mask_cpu
    is_right = (col_indices_flat >= col_center) & mask_cpu

    base_value = (gray_left_value + gray_right_value) / 2
    sample_img_flat = torch.full((n_px_side * n_px_side,), base_value, device='cpu')
    sample_img_flat[is_left] = gray_left_value
    sample_img_flat[is_right] = gray_right_value
    sample_2d = sample_img_flat.reshape(n_px_side, n_px_side)

    # Compute sample masked mean for bipartite
    sample_masked_mean = sample_img_flat[mask_cpu].mean().item()
    sample_title = f'Sample (bipartite: {SAMPLE_GRAY_LEFT*100:.0f}%|{SAMPLE_GRAY_RIGHT*100:.0f}%)\nmask mean={sample_masked_mean:.3f}'
    sample_img_flat_cpu = sample_img_flat

else:
    sample_2d = torch.full((n_px_side, n_px_side), sample_pixel_value.cpu().item(), device='cpu')
    sample_masked_mean = sample_pixel_value.cpu().item()
    sample_title = f'Sample ({SAMPLE_GRAY_LEVEL*100:.0f}% gray)\nmask mean={sample_masked_mean:.3f}'
    sample_img_flat_cpu = torch.full((n_px_side * n_px_side,), sample_pixel_value.cpu().item(), device='cpu')

# Use a single shared diverging scale for all diff panels (across all starting grays)
# Diff is now: Optimized - Sample (only masked pixels matter)
global_max_abs_diff = 0.0
for result_key, result in results.items():
    final_img = result['optimized_img']
    # Compute diff only in masked region
    diff_masked_only = (final_img.cpu() - sample_img_flat_cpu)[mask_cpu]
    global_max_abs_diff = max(global_max_abs_diff, diff_masked_only.abs().max().item())
global_max_abs_diff = max(global_max_abs_diff, 1e-12)

for i, (result_key, result) in enumerate(results.items()):
    # Get images
    initial_img = result.get('initial_img')
    final_img = result['optimized_img']
    ...
    elif USE_NATURAL_SMOOTHED:
        # Shouldn't happen - initial_img should always exist for natural smoothed
        initial_2d = torch.zeros((n_px_side, n_px_side), device='cpu')
    else:
        start_val = (pixel_min + pixel_range * result_key).cpu().item()
        initial_2d = torch.full((n_px_side, n_px_side), start_val, device='cpu')

    final_2d = final_img.cpu().reshape(n_px_side, n_px_side)

    # Compute difference: Optimized - Sample (only in masked region, zero elsewhere)
    diff_2d = torch.zeros_like(final_2d)
    diff_2d[mask_2d] = final_2d[mask_2d] - sample_2d[mask_2d]

    # Get analysis values for this result
    ar = analysis_results[result_key]

    # Plot initial (nudged for natural mode, gray for other modes)
    im0 = axes[i, 0].imshow(initial_2d.numpy(), cmap='gray', vmin=vmin, vmax=vmax)
    if USE_NATURAL_SMOOTHED:
        axes[i, 0].set_title(
            f'Initial (nudged +{NUDGE_AMOUNT})\nmask mean={ar["initial_masked_mean"]:.3f}'
        )
    else:
        axes[i, 0].set_title(
            f'Initial ({result_key*100:.0f}% gray)\nmask mean={ar["initial_masked_mean"]:.3f}'
        )
    axes[i, 0].axis('off')
    plt.colorbar(im0, ax=axes[i, 0], fraction=0.046)

    # Plot sample (target) - same for all rows
    im1 = axes[i, 1].imshow(sample_2d.numpy(), cmap='gray', vmin=vmin, vmax=vmax)
    axes[i, 1].set_title(sample_title)
    axes[i, 1].axis('off')
    ...
    # Plot difference: Optimized - Sample (masked region only)
    ...
    axes[i, 3].set_title(f'Optimized - Sample (masked only)\n[{diff_masked_min:.2f}, {diff_masked_max:.2f}], mean={diff_masked_mean:.3f}')
    axes[i, 3].axis('off')
    plt.colorbar(im3, ax=axes[i, 3], fraction=0.046)

    # Add row label on the left
    if USE_NATURAL_SMOOTHED:
        axes[i, 0].set_ylabel(f'nudge={NUDGE_AMOUNT}', fontsize=12, rotation=0, labelpad=50, va='center')
    else:
        axes[i, 0].set_ylabel(f'Start: {result_key*100:.0f}%', fontsize=12, rotation=0, labelpad=50, va='center')

# Build suptitle based on mode
if USE_NATURAL_SMOOTHED:
    suptitle = (f'Image Optimization: Nudged (+{NUDGE_AMOUNT}) → Original Natural Image\n'
                f'Model R² = {r2:.4f}, {N_ITERATIONS} iterations, lr={LR}, mask={mask.sum().item()} pixels')
elif USE_BIPARTITE_SAMPLE:
    suptitle = (f'Image Optimization: Uniform Gray → Bipartite Sample ({SAMPLE_GRAY_LEFT*100:.0f}%|{SAMPLE_GRAY_RIGHT*100:.0f}%)\n'
                f'Model R² = {r2:.4f}, {N_ITERATIONS} iterations, lr={LR}, mask={mask.sum().item()} pixels')
else:
    suptitle = (f'Image Optimization: Gray Levels → {SAMPLE_GRAY_LEVEL*100:.0f}% Sample (masked mean={sample_pixel_value.item():.3f})\n'
                f'Model R² = {r2:.4f}, {N_ITERATIONS} iterations, lr={LR}, mask={mask.sum().item()} pixels')
...
# Save figure
if USE_NATURAL_SMOOTHED:
    save_path = Path(__file__).parent / f'optimization_natural_nudge{NUDGE_AMOUNT}.png'
elif USE_BIPARTITE_SAMPLE:
    save_path = Path(__file__).parent / 'optimization_bipartite_test.png'
else:
...
if USE_NATURAL_SMOOTHED:
    print(f"Mode: Natural Nudged (nudge={NUDGE_AMOUNT})")
else:
...
for result_key, result in results.items():
    ...
    delta_u = result['U_final'] - result['U_initial']
    if USE_NATURAL_SMOOTHED:
        print(f"\n  Natural Nudged (nudge={NUDGE_AMOUNT}):")
    else:
        print(f"\n  {result_key*100:.0f}% gray (pixel={ar['start_pixel_val']:.4f}):")
