...
from gp_utility_playground import (
    VariationalGP, compute_elbo, evaluate_nd_utility_new, generate_poisson_data,
    DEVICE, DTYPE,
)
...
# Import ArcCosineKernel explicitly from gpytorch_porting
gpytorch_porting_path = Path(__file__).parent.parent.parent / 'gpytorch_porting'
sys.path.insert(0, str(gpytorch_porting_path))
import importlib.util
spec = importlib.util.spec_from_file_location("gpy_kernels", gpytorch_porting_path / "kernels.py")
...
spec2 = importlib.util.spec_from_file_location("gpy_likelihoods", gpytorch_porting_path / "likelihoods.py")
gpy_likelihoods = importlib.util.module_from_spec(spec2)
spec2.loader.exec_module(gpy_likelihoods)
PoissonLikelihood = gpy_likelihoods.PoissonLikelihood
...
with torch.no_grad():
    ...
    utility = evaluate_nd_utility_new(model, test_points, max_r=100)
    ...
    full_utility = evaluate_nd_utility_new(model, eval_flat, max_r=100)
