#!/usr/bin/env python3
"""
run_normalized.py - Run training with NORMALIZED arc-cosine kernel on PNAS data.
Created by Claude.

Copy of run_single_mode.py modified to use ArcCosineKernelNormalized instead of
ArcCosineKernel. For the normalized kernel investigation.

Usage (from gpytorch_porting/):
    python investigations/normalized_kernel/run_normalized.py --mode default_gpy
    python investigations/normalized_kernel/run_normalized.py --mode vargp_direct
...
# ---------------------------------------------------------------------------
# Path setup: this script lives in investigations/normalized_kernel/
# ---------------------------------------------------------------------------
...
from kernels import ArcCosineKernel, ArcCosineKernelNormalized, GRADIENT_MODES
...
def run_single_config(config):
    ...
    if ip_selection == 'pivoted' and mode != 'vargp_old':
        ...
        temp_kernel = ArcCosineKernelNormalized(
            n_px_side=n_px_side,
    ...
    if mode == 'vargp_old':
        raise NotImplementedError(
            "vargp_old mode does not support normalized kernel "
    ...
    # =========================================================================
        ...
        base_kernel = ArcCosineKernelNormalized(
            sigma_0=config['sigma_0'],
            n_px_side=n_px_side,
            eps_0x=eps_0x,
            eps_0y=eps_0y,
...
def main():
    ...
    if args.save_plot == 'auto':
        ...
        save_path = imgs_dir / f"normalized_{args.mode}_M{args.ntilde}.png"
