#!/usr/bin/env python3
"""
run_rbf.py - Run training with LocalRBFKernel on PNAS data.
Created by Claude.

Copy of run_arcsine.py modified to use LocalRBFKernel instead of
ArcSineKernel. For the RBF kernel investigation.

The local RBF kernel is a stationary kernel: k(x,y) = exp(-1/2 (x-y)^T C (x-y))
where C is the same RF-structured matrix as arc-cosine. k(x,x) = 1 for all x
(constant prior variance), so all drive comes from the likelihood (A, lambda0).

Usage (from gpytorch_porting/):
    python investigations/rbf_kernel/run_rbf.py --mode default_gpy
...
from kernels import ArcCosineKernel, LocalRBFKernel, GRADIENT_MODES
...
def run_single_config(config):
    ...
    if mode == 'vargp_old':
        raise NotImplementedError(
            "vargp_old mode does not support LocalRBFKernel "
    ...
    # =========================================================================
        ...
        # Local RBF kernel diagnostic: pairwise distance and kernel value statistics
        with torch.no_grad():
...
def main():
    ...
    if args.save_plot == 'auto':
        ...
        save_path = imgs_dir / f"rbf_{args.mode}_M{args.ntilde}.png"
