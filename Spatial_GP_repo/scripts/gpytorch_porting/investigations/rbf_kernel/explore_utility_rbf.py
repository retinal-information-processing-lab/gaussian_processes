"""
Workbench for exploring utility function behavior with the LocalRBFKernel.
Created by Claude.

Adapted from investigations/understanding_utility/explore_utility.py for the
RBF kernel investigation. Companion script: gradient_rbf.py (gradient ascent study).
...
(hardcoded below) using the LocalRBFKernel.

Key differences from the arc-cosine version:
- Imports from run_rbf.py (not run_single_mode.py) to get LocalRBFKernel
- K(x,x) = 1 for all x (stationary kernel), so ||x||_C is always 1
- No sigma_0 override (sigma_0 has no effect on RBF forward())
- Right panel plots U_DA vs K(x*, x_cond) instead of vs ||x||_C colored by angle
- Added kernel_distance() helper: sqrt(-2*log(K)) = d_C/l
...
Usage:
    ...
    python investigations/rbf_kernel/explore_utility_rbf.py

    # Or import in a script/REPL:
    from explore_utility_rbf import setup, kernel_angle, describe, ...
"""
...
from run_rbf import run_single_config, build_config_from_defaults
...
def kernel_norm(model, x):
    ...
    For LocalRBFKernel, this always returns ~1.0 (constant diagonal).
    Kept as a verification check.
    """
...
def kernel_angle(model, x1, x2):
    ...
    For RBF with K(x,x)=1, this simplifies to arccos(K(x1,x2)).
    """
...
def describe(model, likelihood, x, x_ref=None, label="image"):
    ...
    Args:
        ...
        x_ref: optional reference image for K value, distance, and DA utility
...
def eval_da_conditioned(model, likelihood, X_train, x_cond):
    ...
    Prints a table with DA utility, H_marg, H_cond, and K(x*, x_cond)
    for each image, sorted by DA utility descending.
    """
    print("\n" + "=" * 60)
    ...
    # Compute K(x*, x_cond) for each candidate
    k_vals = []
    for i in range(x_candidates.shape[0]):
        k_vals.append(kernel_value(model, x_candidates[i], x_cond))

    # Sort by DA utility descending
    sorted_idx = utilities.argsort(descending=True)

    print(f"\n  K(x_cond, x_cond) = {kernel_value(model, x_cond, x_cond):.4f}")
...
def plot_da_landscape(model, likelihood, X_candidates, x_cond, save_path,
    ...
    Two panels:
    - Left: H(R) landscape with marginal/conditional scatter connected by lines
    - Right: U_DA vs K(x*, x_cond) -- shows how kernel similarity to conditioning
      image relates to DA utility (the key axis of variation for a stationary kernel)
    ...
    # Compute K(x*, x_cond) for right panel x-axis
    ...
    # === Right panel: U_DA vs K(x*, x_cond) ===

    # Plot training images (circles)
    ax2.scatter(
        k_to_cond[idx_train], U_DA_vals[idx_train],
        s=30, c='tab:blue', edgecolors='white', linewidths=0.3, zorder=3,
    ...
    ax2.set_title('DA utility vs kernel similarity to $x_{cond}$', fontsize=11)
...
def demo():
    ...
    print("\n" + "=" * 60)
    print("PART 2: Same image scaled by c=5")
    print("  For RBF: K(cx, cx) = 1 still, but K(cx, y) changes because")
    print("  (cx-y)^T C (cx-y) != (x-y)^T C (x-y)")
    print("=" * 60)
    ...
    print("\n" + "=" * 60)
    print("PART 3: A different natural image")
    print("=" * 60)
    # Find one at moderate kernel distance
    k_vals = []
    for i in range(min(100, X_pool.shape[0])):
        k = kernel_value(model, x_target, X_pool[i])
        k_vals.append((i, k))
    k_vals.sort(key=lambda x: x[1])
    # Pick one near the median K value
    ...
    print("\n" + "=" * 60)
    print("COMPARISON: Amplitude vs Distance effects (RBF kernel)")
    print("=" * 60)
    ...
    print(f"""
  Key observations (RBF kernel -- stationary, K(x,x)=1):
  - K(x,x) = 1.0 for ALL images -- no norm-driven utility growth.
  - Scaling UP: K(cx, x_target) changes because distance changes, but
    K(cx, cx) = 1 still. Utility depends on how scaling affects the
    distance to conditioning image, not on absolute magnitude.
  - Different images: K(x, x_target) directly encodes proximity in
    C-space. Low K = far away = weak conditioning = low DA utility.
  - Standard utility depends on GP variance, which for RBF is determined
    by proximity to inducing/training points (not by self-kernel value).
""")
    ...
    plot_da_landscape(
        ...
        save_path=_script_dir / 'explore_utility_rbf.png',
    )
