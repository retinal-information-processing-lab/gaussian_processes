"""
Gradient ascent investigation for DA utility with LocalRBFKernel.
Created by Claude.

Adapted from investigations/understanding_utility/gradient_unnormalized.py for the
RBF kernel investigation. Companion to explore_utility_rbf.py (utility workbench).
...
Uses LocalRBFKernel (K(x,x) = 1, stationary), which has no norm-dependent
utility behavior -- gradient ascent should converge toward the target via
distance reduction rather than norm growth.
...
Usage:
    python investigations/rbf_kernel/gradient_rbf.py
"""
