#!/usr/bin/env python3
"""
run_arcsine.py - Run training with ARC-SINE kernel (Williams 1998) on PNAS data.
Created by Claude.

Copy of run_single_mode.py modified to use ArcSineKernel instead of
ArcCosineKernel. For the arc-sine kernel investigation.

The arc-sine kernel saturates at 1 for large inputs (erf activation instead of
ReLU), preventing K(x,x) from growing quadratically with input norm. Same C
matrix and RF structure as arc-cosine.

Usage (from gpytorch_porting/):
    python investigations/arcsine_kernel/run_arcsine.py --mode default_gpy

All parameters from default_params.json, same as the original script.
vargp_old mode is NOT supported (uses old codebase kernel internally).
vargp_direct mode is OUT OF SCOPE for this investigation.
"""
...
# ---------------------------------------------------------------------------
# Path setup: this script lives in investigations/arcsine_kernel/
# ---------------------------------------------------------------------------
_gpytorch_dir = Path(__file__).resolve().parent.parent.parent
...
from kernels import ArcCosineKernel, ArcSineKernel, GRADIENT_MODES
...
def build_config_from_defaults(**overrides):
    ...
    # ------------------------------------------------------------------
    defaults_path = _gpytorch_dir / 'default_params.json'
...
def run_single_config(config):
    ...
    if mode == 'vargp_old':
        raise NotImplementedError(
            "vargp_old mode does not support arcsine kernel "
            "(uses old codebase kernel internally). Use default_gpy or vargp_direct."
        )
    ...
    # =========================================================================
        ...
        # Arc-sine kernel diagnostic: v_x statistics on training images
        with torch.no_grad():
            ...
            v_x = (x_masked @ C_trained * x_masked).sum(dim=-1) + kernel.sigma_0.item()**2
            k_diag = (2.0 / torch.pi) * torch.arcsin(v_x / (1.0 + v_x))
            print(f"\n  Arc-sine kernel diagnostics (training images):")
            print(f"    v_x: min={v_x.min().item():.1f}, median={v_x.median().item():.1f}, max={v_x.max().item():.1f}")
            print(f"    K_sat(x,x): min={k_diag.min().item():.4f}, median={k_diag.median().item():.4f}, max={k_diag.max().item():.4f}")
            print(f"    (K_sat saturates at 1.0 as v_x -> inf)")
...
def main():
    ...
    if args.save_plot == 'auto':
        ...
        save_path = imgs_dir / f"arcsine_{args.mode}_M{args.ntilde}.png"
