...
def main():
    ...
    clip_warnings = {}
    for label, vals in [('A_smoothed', perturbed_rf_vals), ('Final', final_rf_vals)]:
    ...
    # Difference image: final - smoothed (shows what optimizer changed)
    ...
    # 4th image: difference (final - smoothed), diverging colormap
    ax_diff = fig.add_subplot(2, 4, 4)
    diff_abs_max = np.abs(img_diff).max()
