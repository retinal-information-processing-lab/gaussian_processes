#!/usr/bin/env python3
"""
Confirmation run: verify test_r is preserved with rel_tol=1e-7.
...
RESULTS_FILE = os.path.join(SCRIPT_DIR, 'confirmation_rel1e7_results.jsonl')
...
def run_single_cell(cell_id, seed):
    ...
    config['lbfgs_tolerance_change_rel'] = REL_TOL
...
def main():
    ...
    print(f"=== Confirmation: rel_tol={REL_TOL:.0e} vs baseline ===")
    ...
    print(f"\n{'='*70}")
    print(f"COMPARISON: rel_tol={REL_TOL:.0e} vs baseline (abs tol=1e-9)")
    print(f"{'='*70}")
